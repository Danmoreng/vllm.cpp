...
//     model_executor/layers/quantization/modelopt.py:2186-2200
//     (`KMxfp8Static.get_scale_weight_loader`)
//   - the runtime per-32-column dequant:
//     .../utils/mxfp8_utils.py:226-239 (`dequant_mxfp8_to_bf16`)
//   - the dynamic activation quantizer:
//     .../utils/mxfp8_utils.py:114-141 (`_mxfp8_quant_triton_kernel`), whose
//     amax/bias/clamp are identical to :56-70 (`_mxfp8_e4m3_quantize_torch`)
//   - the emulation linear arm:
//     model_executor/kernels/linear/mxfp8/emulation.py:29-66
//   - the checkpoint scale name split:
//     models/deepseek_v4_1/nvidia/model.py:871-882
// See deepseek_v4_1_mxfp8.h for the full semantics, the two named divergences,
...
std::vector<uint8_t> ExpandMxfp8CheckpointScale(const uint8_t* ckpt_scale,
                                                ...
                                                int64_t block_rows,
                                                int64_t block_cols) {
  // modelopt.py:2191-2194 raises NotImplementedError on exactly these two, and
...
void DequantMxfp8Rows(const uint8_t* weight_f8, const uint8_t* scale_e8m0, int64_t N,
  ...
  for (int64_t n = 0; n < N; ++n) {
    ...
    // expansion above. mxfp8_utils.py:232-236 broadcasts one scale per
    // (row, 32-column block).
    const uint8_t* srow = scale_e8m0 + n * scale_cols;
    for (int64_t k = 0; k < K; ++k) {
      // `x.to(f32) * exp2(scale.to(f32) - 127)` (mxfp8_utils.py:228,234,236).
...
void DequantMxfp8ToBf16(const uint8_t* weight_f8, const uint8_t* scale_e8m0, int64_t N,
  ...
  // `.to(torch.bfloat16)` at mxfp8_utils.py:239 — the narrowing happens ONCE, at
...
void QuantizeMxfp8E4m3(const float* x, int64_t M, int64_t K, uint8_t* out_q,
  ...
  VT_CHECK(K % kMxfp8BlockSize == 0,
           ...
           "(mxfp8_utils.py:54)");
  // `TINY = torch.finfo(torch.float32).tiny` (mxfp8_utils.py:196), the smallest
  // NORMAL float, which is what `amax.clamp(min=...)` uses at :61.
  ...
  for (int64_t m = 0; m < M; ++m) {
    for (int64_t b = 0; b < blocks; ++b) {
      ...
      // the suite at 13/13 with the binary proved changed, because `log2(0)` is
      ...
      // e4m3 range rather than at 1.0; :124-126 says why — otherwise the small
      ...
      // `rescale = exp2(127 - sb); xq = x * rescale` (mxfp8_utils.py:136-137).
      // MULTIPLY by the reciprocal, not divide by `exp2(sb - 127)`: upstream's
      // own comment at :130-135 says a `sb == 0` block makes that divisor
      ...
      // for every reachable sb the two forms agree bit-for-bit.
...
void Mxfp8LinearEmulation(const float* x, int64_t M, int64_t K, const uint8_t* weight_f8,
  ...
  VT_CHECK(M > 0 && N > 0, "MXFP8 emulation linear: dims must be positive");

  // emulation.py:44-48: `dequant_mxfp8_to_bf16` ONCE, at load, replacing the
  ...
  // emulation.py:60-63: a plain `F.linear(x, weight, bias)` — the emulation arm
  ...
  for (int64_t m = 0; m < M; ++m) {
    ...
    for (int64_t n = 0; n < N; ++n) {
      ...
      float acc = 0.0F;
      for (int64_t k = 0; k < K; ++k) acc += xrow[k] * vt::BF16ToF32(wrow[k]);
      out_f32[m * N + n] = bias != nullptr ? acc + bias[n] : acc;
    }
  }
}
