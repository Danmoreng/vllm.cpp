...
capture_dir="${GPTQ_CAPTURE_DIR:-$repo_dir/B70_GPTQ_INT4_Plan/evidence/gptq00_runtime_capture}"
...
[[ "${source_pin[0]:-}" == "mikeinnyc/Qwen3.8-27B-GPTQ-Int4-sym-G128-MTP-BF16" &&
   "${source_pin[1]:-}" == "a47b0c6f0d756bc394c4cc629d5b0ded1acc7001" ]] || {
...
mkdir -p "$capture_dir"
[[ ! -e "$capture_dir/routes.jsonl" ]] || {
  echo "Capture directory already contains routes: $capture_dir" >&2
  exit 2
}
printf '{}\n' > "$capture_dir/current_prompt.json"

exec docker run --rm --pull=never --name b70-gptq-reference-capture \
  ...
  --mount "type=bind,source=$repo_dir/tools/gptq_reference/capture_site,target=/capture,readonly" \
