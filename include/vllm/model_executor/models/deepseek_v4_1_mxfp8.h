...
//
//   modelopt.py:2186-2200 (`KMxfp8Static.get_scale_weight_loader`)
...
// (modelopt.py:2225-2234) and then sets `layer.weight_block_size = [1, 32]`
...
// `NotImplementedError` (modelopt.py:2191-2194).
//
...
//                                     (mxfp8_utils.py:126-141).
//   (4) Mxfp8LinearEmulation        — the EMULATION linear arm: dequant the
//                                     weight to bf16 once, then a plain bf16
//                                     linear (emulation.py:29-66).
...
// `vllm::F32ToF8E4M3`
// (`compressed_tensors/nvfp4_emulation.h:50`), which already mirrors torch
...
//   `_mxfp8_e4m3_quantize_torch` (mxfp8_utils.py:65-70) DIVIDES by
//   `exp2(sb - 127)`; the Triton kernel (mxfp8_utils.py:126-137) MULTIPLIES by
...
//   zero block into `0/0 == NaN`. Both are exact powers of two, so for every
//   reachable `sb` they agree bit-for-bit. We mirror the MULTIPLY, which is the
...
//   divide leaves the unit suite at 13/13 with the binary proved changed. It is
...
//                                    modelopt.py:2186-2200
//                                    (`KMxfp8Static.get_scale_weight_loader`)
//   DequantMxfp8To{F32,Bf16}     <-  .../utils/mxfp8_utils.py:226-239
//                                    (`dequant_mxfp8_to_bf16`)
//   QuantizeMxfp8E4m3            <-  .../utils/mxfp8_utils.py:126-141
//                                    (`_mxfp8_quant_triton_kernel`), whose
//                                    amax/clamp/bias mirror
//                                    :56-70 (`_mxfp8_e4m3_quantize_torch`)
...
// `MXFP8_BLOCK_SIZE` (mxfp8_utils.py:12). The scale group is 32 ELEMENTS ALONG
// K, in both the checkpoint and the runtime layout.
inline constexpr int64_t kMxfp8BlockSize = 32;

// `torch.finfo(torch.float8_e4m3fn).max` (mxfp8_utils.py:62). The quantizer puts
// the block amax at the TOP of this range rather than at 1.0, or the small
// elements of the block land in the fp8 subnormals (mxfp8_utils.py:124-126).
inline constexpr float kMxfp8Fp8Max = 448.0F;

// (1) `KMxfp8Static.get_scale_weight_loader` (modelopt.py:2186-2200): expand the
...
//
// REFUSES BY NAME, mirroring modelopt.py:2191-2194: `block_cols` must be exactly
...
// (2) `dequant_mxfp8_to_bf16` (mxfp8_utils.py:226-239) on the RUNTIME layout:
//
...
// emulation kernel stores bf16 (emulation.py:46-48), so an f32 weight buffer
...
// nothing is stored, the kernel quantizes at runtime — modelopt.py:2251-2263,
// :2278). Ported from the Triton kernel `_mxfp8_quant_triton_kernel`
// (mxfp8_utils.py:114-141), whose amax/bias/clamp are identical to the torch
// reference at :56-70:
//
...
// (4) `EmulationMxfp8LinearKernel` (emulation.py:29-66). The emulation backend
...
//
//   x       [M, K]     f32 (the caller's model dtype, widened)
