...
The response carries one answer per question under `answers`, keyed by question
id, plus `usage` and `latency_ms`:

```json
{"model":"laya",
 "answers":{"pick":{"type":"choice","choice":"person",
   "probabilities":{"person":0.6681,"organization":0.3319},
   "confidence":0.3362}},
 "usage":{"input_tokens":7,"output_tokens":0},
 "latency_ms":12.34}
```

`/v1/systemone/permute` re-runs one choice question under `n_perm` option
orders and reports per-order probabilities and their spread. Its body wraps a
...
A server running cua-s1-forms registers `/v1/score`. A server without it
answers 404. The request carries `context`, the text to score, and `options`,
an array of candidate strings:

```sh
curl http://localhost:8000/v1/score \
  -H 'Content-Type: application/json' \
  -d '{"model":"cua-s1-forms","context":"Is this field required?",
       "options":["yes","no"]}'
```

The response carries `probabilities`, one per option and rounded to four
places, the index of the highest under `winner`, a `confidence` margin, and
timing:

```json
{"model":"cua-s1-forms",
 "probabilities":[0.8123,0.1877],
 "winner":0,
 "confidence":0.6246,
...
| Nail-Qwen3.6-35B-A3B-MTP IQ4_XS | `Nail-Qwen3.6-35B-A3B-MTP-IQ4_XS.gguf` | 19,389,012,960 bytes (19.39 GiB); 753 tensors, histogram `{BF16: 2, F32: 308, IQ4_XS: 391, Q5_K: 51, Q6_K: 1}` | Developer's best recollection is `peculiar-ragdoll/Nail-Qwen3.6-35B-A3B-GGUF-MTP` (2026-09-12), unverified against this file and no revision recorded at fetch time; the sha256 beside this, not the repo name, is the pin. The GGUF's own `general.base_model.0.repo_url`/`general.quantized_by` name the base as `Qwen/Qwen3.6-35B-A3B`, quantized by Unsloth | `aeff61097613c4d8e9f418572d58afd14bf5771de236e1dd50869dd809343a23` | This row's motivating and gate checkpoint for `KERNEL-QUANT-CIQ-GEMM-ROCM-IQUANT` (#1940, pull request #3029): the IQ4_XS keep-quant ROCm GEMM. Measured on `isravale` (RX 9060 XT, `gfx1200`) under `flock ${GPU_LOCK:-$HOME/gpu.lock}`: `--fit` keeps 391 IQ4_XS tensors plus 51 Q5_K and 1 Q6_K compressed, places 15 of 40 layers' routed experts on CPU under a 13 GB device budget, and generates `" Paris. The capital of Germany is Berlin. The capital of Italy is Rome."` -- byte-identical to a `--device cpu` run of the same prompt at greedy decoding, 16/16 completion tokens agreeing. Peak RSS **measured** (`tools/bench/sample_process_memory.py`, `/proc/<pid>/smaps_rollup`, 200 ms interval): 20,135,752 KiB (~19.20 GiB) | The MTP head (`blk.40.*`, 20 tensors, 449,689,600 B) is loaded only under `speculative-config method "mtp"` and is skipped on this trunk-only reload, matching the pinned llama.cpp `b10451` oracle's own denominator |
...
| Laya, System-1 decision model | model directory | Owed: no byte has been fetched or hashed | `convaiinnovations/laya` @ `aa8c91ca088ec597df95a0d1c76b3063cb2ae5e8` | Owed, as above | System-1 decisions through `/v1/systemone` for `choice`, `score`, and `noul` question types | No refused arm recorded |
| kev 0.8B | pre-converted checkpoint from `scripts/convert-kev.py` | Owed: no byte has been fetched or hashed | `jaredpalmer/kev-0.8b` @ `54f4f877`, base `Qwen/Qwen3.5-0.8B-Base` @ `dc7cdfe2` | Owed, as above | System-1 decisions through `/v1/systemone` | Requires `scripts/convert-kev.py` pre-conversion, which merges a rank-16 LoRA adapter into the base weights and extracts the PointerHead head weights |
| cua-s1-forms | model directory | Owed: no byte has been fetched or hashed | `cua-ai/cua-s1-forms` @ `f54adbf447f4` | Owed, as above | Option scoring through `/v1/score` | No refused arm recorded |
